...
export function BrightButton ({text, action, link}) {
    if (link) {
        return(
            <a href={link} className="w-fit text-nowrap px-10 py-4 rounded-[10px] bg-bright text-white font-base text-xl shadow-small">
                {text}
            </a>
        )
    }
    return(
        <button type="submit" onSubmit={action} className="w-fit px-10 py-4 rounded-[10px] bg-bright text-white font-base text-xl shadow-small">
            {text}
        </button>
    )
}