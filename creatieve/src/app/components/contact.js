...
export default function Contact () {
    ...
    return (
        <section id="5" className="shadow-section bg-white py-14 max-[1100px]:py-10 min-h-screen">
            <div className="wrapper flex flex-col gap-y-14 items-center">
                <div className="flex gap-10 max-[1100px]:gap-[30xp] items-center">
                    <p className="text-active-base font-base text-4xl max-[1100px]:text-[28px]"><span className="text-bright font-attention">Свяжитесь с нами</span>, если</p>
                    <ul className="flex flex-col gap-y-5">
                        {reasons.map(reason => <Reason key={reason.id} reason={reason}/>)}
                    </ul>
                </div>
                <div className="flex items-center gap-10 max-[1100px]:gap-5 w-full">
                    <form id='orderCallForm' className="flex w-full flex-col items-start gap-y-5" onSubmit={(e) => sendFormData(e)}>
                        {isSuccess[0] && <label className="text-landingfrom text-base max-[1100px]:text-sm">Вы заказали звонок на {isSuccess[1]}</label>}
                        <div className="grid grid-cols-2 max-[1100px]:grid-cols-1 w-full gap-5 max-[1100px]:gap-2.5">
                            <div className="flex flex-col gap-y-5 max-[1100px]:gap-y-2.5">
                                {fields.map(field => <InputField field={field} key={field.id}/>)}
                                {error != null && <label className="text-red-600 text-base max-[1100px]:text-sm">{error}</label>}
                            </div>
                    ...
                    <div className="w-min h-[400px] flex flex-col gap-y-2.5 items-center">
                        ...
                        <p className="text-base max-[1100px]:text-sm font-base text-active-base">или</p>
                        <div className="h-full rounded w-px bg-active-base"></div>
                    </div>
    ...
    if (field.name != 'time') {
        return (
            <>
                ...
                :
                <>
                    <label className="text-base max-[1100px]:text-sm font-base text-dark-base">{field.placeholder}</label>
...
function Reason ({reason}) {
    return <li className="text-active-base font-base text-xl max-[1100px]:text-base">{reason.reason}</li>
}